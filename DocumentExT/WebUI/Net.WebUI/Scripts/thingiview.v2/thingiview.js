var Thingiview = function (dom_element, desired_width, desired_height) {
...
Thingiview.prototype._init = function (dom_element) {
    ...
    if (!Detector.webgl) { Detector.addGetWebGLMessage({ "parent": this.container }); return; }
...
Thingiview.prototype._initHTMLControls = function () {
    ...
    //this.fullscreenBtn.setAttribute("class", "thing-page-image-opts");
    //this.fullscreenBtn.innerHTML = '<button type="button" class="largeview-btn" title="Toggle Largeview"></button>';
    //this.fullscreenBtn.style.position = "absolute"; this.fullscreenBtn.style.top = "10px";
    //this.fullscreenBtn.style.right = "10px";
    //this.fullscreenBtn.addEventListener('click', function () {
    //    self.toggleFullscreen();
    //});
    //this.toggleThingiviewBtn = document.createElement("div");
    //this.toggleThingiviewBtn.setAttribute("class", "thing-page-image-opts");
    //this.toggleThingiviewBtn.innerHTML = '<button type="button" class="thingiview-btn enabled" title="Exit Thingiview"><span>Exit</span></button>';
    //this.toggleThingiviewBtn.style.position = "absolute";
    //this.toggleThingiviewBtn.style.top = "10px";
    //this.toggleThingiviewBtn.style.right = "50px";
    //this.toggleThingiviewBtn.addEventListener('click', function () {
    //    self.hide(); if (self.fullscreen)
    //        self.toggleFullscreen();
    //});
    //this.container.appendChild(this.fullscreenBtn);
    //this.container.appendChild(this.toggleThingiviewBtn);
}
...
Thingiview.prototype.render = function () {
    if (!this.visible)
        return; var now = Date.now(); if (this.lastRenderTime == undefined)
            ...
            this.timeElapsed = now - this.lastRenderTime; this.lastRenderTime = now; this.controls.dirty = false; this.controls.update(this.timeElapsed); if (this.scene.face_count < 100000) {
                this.reflectCamera.position.z = -this.camera.position.z; this.reflectCamera.position.y = this.camera.position.y; this.reflectCamera.position.x = this.camera.position.x; this.scene.traverse(function (object) {
...
Thingiview.prototype.toggleFullscreen = function () {
    if (this.fullscreen) { this.fullscreen = false; document.body.removeChild(this.container); this.parent_element.appendChild(this.container); this.container.style.position = "static"; this.container.style.top = "0px"; this.container.style.width = this.desired_width + "px"; this.container.style.height = this.desired_height + "px"; this.toggleThingiviewBtn.style.top = "10px"; this.fullscreenBtn.style.top = "10px"; this.fullscreenBtn.childNodes[0].setAttribute("class", "largeview-btn"); document.body.removeChild(this.fullscreenBtn); document.body.removeChild(this.toggleThingiviewBtn); this.container.appendChild(this.fullscreenBtn); this.container.appendChild(this.toggleThingiviewBtn); this.resize(parseInt(this.container.style.width), parseInt(this.container.style.height)); } else {
        this.fullscreen = true; this.parent_element.removeChild(this.container); document.body.appendChild(this.container); this.container.style.position = "absolute"; this.container.style.top = "61px"; this.container.style.width = "100%"; this.container.style.height = "608px"; if (typeof (mw) != 'undefined')
